        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.msg == "加入购物车成功") {
                    ...
                    else if(data.kehuid=="1")
                    {location.href = "/kehumanage/kehubuycart.html";}
    ...
    $("#item-sec .table_data .delete a").click(function () {
        var $priceNum = $(this).attr("data-id");
         ...
         $("#ahtml").click(function(){
           deleteCake($priceNum);
           $(id).parent().parent().remove();totalPrice();
         });        
    })
    //删除商品
...
window.onload=function (){
	...
	function autoChange(){
			index++;
			if(index>1){index=0;}else if(index<0){index=2;}
			change();		
		}			
	function change(){
		clearTimeout(autoId);
...
//	$("#zxzhifu").click(function(){
//    //alert(1)
//    //$("#zxzhifu").attr("checked","checked");//打勾
//		var box=350;
//		var th=$(window).scrollTop()+$(window).height()/1.6-box;
//		var h=document.body.clientHeight;
//		var rw=$(window).width()/2-box;
//		
//$(".showbox").animate({top:th,opacity:'show',width:690,height:430,right:rw},500);
//     $("#zhezhao").css({
...
//            $(".showbox .link_btn").click(function(){
//			$(this).parents(".showbox").animate({top:0,opacity:'hide',width:0,height:0,right:0},500);
//			$("#zhezhao").css("display","none");

//            var DefrayManner = escape($("input[name='DefrayManner']:checked").val());
//             if(DefrayManner!="undefined")
//             {
//               var str=$("input[name='DefrayManner']:checked").parents("label").val();
//               alert(str);
//                 // alert(DefrayManner);
//               //$("#zxzhifu").attr("checked","checked");//打勾
//             }
