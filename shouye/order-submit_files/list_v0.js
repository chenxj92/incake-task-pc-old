...
var ShaCheckOrder = function () {
    ...
    var psBdate = $("#hid_psBdate").val();
    var psEdate = $("#hid_psEdate").val();
    var start = new Date(psBdate.replace("-", "/").replace("-", "/"));
    var end = new Date(psEdate.replace("-", "/").replace("-", "/"));
    var pstime = new Date(peisongdate.replace("-", "/").replace("-", "/"));
    var errMessage = $("#hid_errMessage").val();

    if (liuyi == "1" && (pstime < start || pstime > end)) {
        CommenMsg(errMessage);
        return false;
    }
    //促销活动 结束
    ...
    $.ajax({
        ...
        data: { "isjiaohang": $("#hid_Isyunfen").val(), "ischecked": liuyi, "fapiao": fapiao, "cjson": cjson, "CashPay": $("#Sp_Pay").html(), "yunfei": $("#hid_yunfei").val(), "InValue": $("#hid_InValue").val(), "OtherValue": $("#hid_OtherValue").val(), "card1": $("#card_1").html(), "PcValue": $("#hid_PcValue").val(), "qgid": $("#hid_qgid").val(), "tgid": $("#hid_tgid").val(), "packid": $("#hid_packid").val(), "json": $("#hid_json").val(), "huodong": $("#Sp_huodong").html(), "discount": $("#hid_zk").val(), "product": $("#hid_p").val(), "vip": $("#hid").val(), "YouHui": $("#Sp_YouHui").html(), "Active": $("#Sp_Active").html(), "code": SN, "card": $("#card_0").html(), "Total": $("#Sp_Total").html(), "payType": payType, "DefrayManner": DefrayManner, "SysNo": SysNo, "peisongtime1": peisongtime1, "peisongtime2": peisongtime2, "peisongdate": peisongdate, "peisongtime1": peisongtime1, "beizhu": beizhu, "benzhuRong": benzhuRong, "hk_type": hk_type, "hekareiRong": hekareiRong, "hk_type": hk_type, "hekareiRong": hekareiRong, "annexarticle": annexarticle, "fp_heka": fp_heka, "dinghuo": dingHuo, "shouHuo": shouHuo, "sha": "0", "Invoice": Invoice, "fpkey": fpkey, "birthday": birthday, "buchaprice": ChaPrice, "panic": $("#panic").val() },
        success: function (data) {
            if (data.msg == "订单生成成功") {
                $("#hid_order").val("1");
                ...
                CommenMsg("<p style='color:Red;'>[蛋糕系列]</p>" + data.endhour + "以后不能提交次日订单，请您重新选择收货日期，谢谢！");
            }
            else if (data.msg == "2") {
                CommenMsg("<p style='color:Red;'>[下午茶系列]</p>" + data.endhour + "点以后不能提交次日订单，请您重新选择收货日期，谢谢！");
            }
            else if (data.msg == "3") {
                CommenMsg("<p style='color:Red;'>[Fly Incake]</p>" + data.endhour + "点以后不能提交次日订单，请您重新选择收货日期，谢谢！");
            }
...
var CheckCard = function (codetype) {
    ...
    $("[name='youhui']").attr("checked", false); //促销活动 活动不共享
    changepos("");
...
var ordercardlist = function (codetype) {
    $.ajax({
        url: "../WebPage/InvoicePage.aspx", dataType: "json", type: "post", timeout: "10000",
        data: { "type": "_cardlist", "cardtype": codetype, "sn": $("#hidSN").val() },
        success: function (data) {
...
//促销活动
function fcheckbox() {
    var box = document.getElementsByName("youhui");
    var result = "";
...
//促销活动
var CheckYouHui = function (active) {
    //获取选中项 只能选中一个活动 
    active = fcheckbox();
    getRadioBoxValue("DefrayManner", "ZFB")
    CheckChooseQuan(6); //活动不共享
    $.ajax({
        url: "../WebPage/InvoicePage.aspx", dataType: "json", type: "post", timeout: "10000",
        data: { "type": "_youhiu", "EmpNo": active, "sha": $("#sha").val(), "cart": $("#hid_json").val() },
        success: function (data) {
            if (data.msg == "1") {
                $(".syouhui").attr("checked", false);
                $("#Sp_YouHui").html("0"); //我的优惠券清空
...
var changepos = function (defa) {
    if (defa != "") {
        $("[name='youhui']").attr("checked", false); //促销活动
        CheckChooseQuan(6); //活动不共享
    }
    $.ajax({
        url: "../WebPage/InvoicePage.aspx", dataType: "json", type: "post", timeout: "10000",
...
var CheckOrder = function () {
    ...
    //小蛋糕兑换专区 配送地址仅限内环
    if ($("#hid_Isyunfen").val() == "1") {
        var $yunfei = $("#yunfen").attr("data");
        if (parseInt($yunfei) > 0) {
            CommenMsg("您所选择的收货地址，不支持运费配送，请选择免运费配送地址");
            return false;
        }
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.msg == "0") {
                $.ajax({
                    url: "../WebPage/AddOrder.aspx", dataType: "json", type: "post", timeout: "10000",
                    data: { "isjiaohang": $("#hid_Isyunfen").val(), "ischecked": liuyi, "fapiao": fapiao, "cjson": Consignee, "CashPay": $("#Sp_Pay").html(), "InValue": $("#hid_InValue").val(), "OtherValue": $("#hid_OtherValue").val(), "card1": $("#card_1").html(), "PcValue": $("#hid_PcValue").val(), "qgid": $("#hid_qgid").val(), "tgid": $("#hid_tgid").val(), "packid": $("#hid_packid").val(), "json": $("#hid_json").val(), "huodong": $("#Sp_huodong").html(), "discount": $("#hid_zk").val(), "product": $("#hid_p").val(), "vip": $("#hid").val(), "YouHui": $("#Sp_YouHui").html(), "Active": $("#Sp_Active").html(), "code": SN, "card": $("#card_0").html(), "Total": $("#Sp_Total").html(), "payType": payType, "DefrayManner": DefrayManner, "SysNo": SysNo, "peisongtime2": peisongtime2, "peisongdate": peisongdate, "peisongtime1": peisongtime1, "beizhu": beizhu, "benzhuRong": benzhuRong, "hk_type": hk_type, "hekareiRong": hekareiRong, "hk_type": hk_type, "hekareiRong": hekareiRong, "annexarticle": annexarticle, "fp_heka": fp_heka, "shouHuo": shouHuo, "Invoice": Invoice, "fpkey": fpkey, "birthday": birthday, "buchaprice": ChaPrice, "panic": $("#panic").val(), "paniccake": $("#hid_paniccake").val() },
                    success: function (data) {
                        if (data.msg == "订单生成成功") {
                            $("#hid_order").val("1");
                            location.href = data.url;
                        ...
                        else if ($("#hid_Isyunfen").val() == "1" && data.msg == "4") {
                            CommenMsg(data.endhour);
                        }
                        else if (data.msg == "已售完") {
                            CommenMsg(data.name);
                        }
